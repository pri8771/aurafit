...
- Existing unit tests for scoring, color, persistence, export, and the full-free-product
  guard (`FullFreeProductTests`, DEC-006).
...
- Interrupted analysis/export and low storage.
...
## Automated evidence — 2026-08-18

- Full Debug test run on iPhone 17 Pro simulator, iOS 26.4.1, candidate `1.0 (3)`:
  93 unit/integration tests and 1 scan/import-to-result UI smoke test passed (94/94).
- `EntitlementManagerTests` (11) and the daily-rollover model test were deleted with the
  tier machinery; `FullFreeProductTests` (5) was added (DEC-006).
- Evidence: `quality/evidence/release/1.0-3-full-free/README.md`.
...
Execute `AURA-QA-002`, `AURA-QA-004`, and `AURA-QA-005` (`AURA-QA-010` is N/A by DEC-006) from
...
`AURA-QA-009`; do not silently substitute simulator evidence for camera, thermal,
...
- App Store Connect privacy declarations require release configuration.
